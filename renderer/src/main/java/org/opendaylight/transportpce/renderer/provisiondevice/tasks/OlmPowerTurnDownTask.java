/*
 * Copyright © 2023 AT&T and others.  All rights reserved.
 *
...
public class OlmPowerTurnDownTask implements Callable<OLMRenderingResult> {
    ...
    public OlmPowerTurnDownTask(String serviceName, String path, TransportpceOlmService olmService,
        ServicePathInputData servicePathInputData, Notification notification) {
    ...
    public OLMRenderingResult call() throws Exception {
        ...
        if (result == null) {
            LOG.error("Service power turn down failed on {} path for service {}!", path, serviceName);
        ...
        if (ResponseCodes.SUCCESS_RESULT.equals(result.getResult().getResult())) {
            LOG.info("OLM power turn down finished successfully on {} for service {}", path, serviceName);
            return OLMRenderingResult.ok();
        } else {
            LOG.error("Service power turn down failed on {} for service {}!", path, serviceName);
            return OLMRenderingResult.failed("Operation Failed");
        }
